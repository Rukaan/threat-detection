...
# STEP 6 — email dispatch (written, NOT deployed)

`flink/06-email-agent.sql` in the repo adds a fourth layer: a streaming agent that
emails HIGH/CRITICAL alerts through Zapier MCP.

    CREATE CONNECTION zapier_mcp_connection  'type' = 'mcp_server'
    CREATE TOOL       zapier_email_tool      'type' = 'mcp', allowed_tools = 'gmail_send_email'
    CREATE AGENT      email_dispatch_agent   USING MODEL threat_triage USING TOOLS zapier_email_tool
    INSERT INTO alert_dispatch_log ... LATERAL TABLE(AI_RUN_AGENT(...))

It reuses the Bedrock model from step 3, so no second model is needed. The dispatch
`INSERT` filters on `severity IN ('HIGH','CRITICAL')` and
`likely_false_positive <> 'true'` — without that gate the benign control case would
page a human on every mistyped password.

Not run against a live endpoint. Fill `<ZAPIER_MCP_SSE_URL>`, `<ZAPIER_MCP_API_KEY>`
and `<SOC_INBOX>`, and confirm the Gmail tool name your Zapier MCP server actually
exposes — `gmail_send_email` is typical but configuration-dependent.
...
- Output tables feeding `ML_PREDICT` or `AI_RUN_AGENT` must be `changelog.mode =
  'append'` with NO primary key. With an upsert table the inference statement fails:
  `The column(s): response (generated by non-deterministic function: ...) can not
  satisfy the determinism requirement for correctly processing update message`.
  Flink will not re-run a non-deterministic function over retracted/re-emitted rows.
  This cost a full rebuild of `security_signals` and `threat_alerts` mid-session.
- `CREATE CONNECTION` returning `HTTP 403: Secrets on CONNECTION have expired or are
  invalid` is the AWS credentials, not the model id or endpoint (a bad model id gives
  404). Recover with `ALTER CONNECTION ... SET (...)` then recreate the statement —
  a running job does not reload secrets.
